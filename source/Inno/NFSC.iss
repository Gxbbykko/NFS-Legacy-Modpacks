...
#define MoviesUrl "https://drive.usercontent.google.com/download?id=1InZybn0zVCYOaFTy4fOMEyTbnMymAR1k&export=download&confirm=t&uuid=d6541b23-20f2-4e0f-9fc4-0350829bbad6"
#define MoviesArchiveName "NFSC_MOVIES.7z"
#define MoviesExtractFolder "NFSCMoviesExtract"
#define MoviesSourceSubDir "NFSC\MOVIES"
#define MoviesExpectedMinSize 4800000000

#define ProjectRoot "C:\Users\Gabriel\Desktop\NFSC_Modpack"
...
SetupIconFile={#ImagesDir}\NFS_icon.ico
...
[Icons]
Name: "{app}\_LegacyInstaller\Restore Carbon Legacy Modpack"; Filename: "{app}\_LegacyInstaller\LegacyUI\LegacyUI.exe"; Parameters: "--target ""{app}"" --mode uninstall --game {#GameId}"; WorkingDir: "{app}\_LegacyInstaller\LegacyUI"; IconFilename: "{app}\_LegacyInstaller\LegacyUI\LegacyUI.exe"
...
begin
  ...
  WriteLegacyUIState('extracting', '20', 'Extracting Carbon archive payload...');
  ...
  repeat
    ...
    WriteLegacyUIState('extracting', IntToStr(ProgressPercent), 'Extracting Carbon archive payload...');
  ...
  WriteLegacyUIState('copying', '65', 'Installing Carbon modpack files...');
  Result := True;
end;
      ...
      repeat
        ...
        begin
          ...
          begin
            ...
            if not FileExists(DestPath) then
            begin
              if (NewFilesManifestPath <> '') and (not ShouldSkipManifest(RelPath)) then
            ...
            if (ManifestPath <> '') and (not ShouldSkipManifest(RelPath)) then
...
begin
  ...
  WriteLegacyUIState('downloading', '78', 'Downloading Carbon MOVIES package...');
...
begin
  ...
  WriteLegacyUIState('extracting', '86', 'Extracting Carbon MOVIES package...');
  ...
  Params := 'x "' + MoviesArchivePath + '" -o"' + MoviesExtractPath + '" -y';
  ...
  begin
    ...
    CreateErrorReport('Expected MOVIES source folder missing: ' + AddBackslash(MoviesExtractPath) + '{#MoviesSourceSubDir}');
    Exit;
  end;
...
begin
  ...
  WriteLegacyUIState('copying', '92', 'Installing Carbon MOVIES package...');

  MoviesBaseDir := AddBackslash(ExpandConstant('{tmp}\{#MoviesExtractFolder}')) + 'NFSC';
  SourceMoviesDir := AddBackslash(MoviesBaseDir) + 'MOVIES';
  ...
  begin
    SetRestoreDataAttributes(GetActiveInstallDir());
    WriteLegacyUIState('finalizing', '95', 'Carbon MOVIES package installed.');
  end
  else
  begin
    WriteLegacyUIState('error', '100', 'Failed to copy MOVIES folder into the game directory.');
...
begin
  ...
  begin
    ...
    WriteLegacyUIState('removing', '25', 'Removing installed Carbon Legacy Modpack files...');
    DeleteFilesFromManifest(GameDir, NewFilesManifestPath);

    if DirExists(BackupDir) then
    begin
      WriteLegacyUIState('restoring', '60', 'Restoring original Carbon game files...');
      ...
      WriteLegacyUIState('complete', '100', 'Rollback complete. Original Carbon files restored.');
    end
    else
    begin
