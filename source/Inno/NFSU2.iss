...
#define MoviesUrl "https://drive.usercontent.google.com/download?id=1uvbG5W7-AafAEgEBKlZAv8dYealf1Doh&export=download&confirm=t&uuid=4a33769d-9162-4c3f-b19b-105d52441479"
#define MoviesArchiveName "NFSU2_MOVIES.7z"
#define MoviesExtractFolder "NFSU2MoviesExtract"
#define MoviesSourceSubDir "NFSU2\MOVIES"
#define MoviesExpectedMinSize 900000000

#define ProjectRoot "C:\Users\Gabriel\Desktop\NFSU2_Modpack"
...
SetupIconFile={#ImagesDir}\NFSU2_icon.ico
...
[Icons]
Name: "{app}\_LegacyInstaller\Restore Underground 2 Legacy Modpack"; Filename: "{app}\_LegacyInstaller\LegacyUI\LegacyUI.exe"; Parameters: "--target ""{app}"" --mode uninstall --game {#GameId}"; WorkingDir: "{app}\_LegacyInstaller\LegacyUI"; IconFilename: "{app}\_LegacyInstaller\LegacyUI\LegacyUI.exe"
...
begin
  ...
  WriteLegacyUIState('extracting', '20', 'Extracting Underground 2 archive payload...');
  ...
  repeat
    ...
    ProgressPercent := Integer(ExtractedSize div 51747240);
    ...
    WriteLegacyUIState('extracting', IntToStr(ProgressPercent), 'Extracting Underground 2 archive payload...');
  ...
  WriteLegacyUIState('copying', '65', 'Installing Underground 2 modpack files...');
  Result := True;
end;
...
begin
  ...
  WriteLegacyUIState('downloading', '78', 'Downloading Underground 2 MOVIES package...');
  ...
  begin
    WriteLegacyUIState('error', '100', 'MOVIES package download failed.');
  ...
  if not FileSize(MoviesArchivePath, Size) then
  begin
    WriteLegacyUIState('error', '100', 'Could not verify MOVIES package size.');
    CreateErrorReport('Could not read MOVIES archive size: ' + MoviesArchivePath);
    Exit;
  end;

  if Size < {#MoviesExpectedMinSize} then
  begin
    WriteLegacyUIState('error', '100', 'MOVIES package download was incomplete or invalid.');
    CreateErrorReport(
      'MOVIES archive is too small. Download likely returned an HTML warning page. Size: ' +
      IntToStr(Size)
    );
    Exit;
  end;
...
begin
  ...
  WriteLegacyUIState('extracting', '86', 'Extracting Underground 2 MOVIES package...');
...
begin
  ...
  WriteLegacyUIState('copying', '92', 'Installing Underground 2 MOVIES package...');

  SourceMoviesDir :=
    AddBackslash(ExpandConstant('{tmp}\{#MoviesExtractFolder}')) + '{#MoviesSourceSubDir}';

  SourceMoviesBaseDir :=
    AddBackslash(ExpandConstant('{tmp}\{#MoviesExtractFolder}')) + 'NFSU2';
  ...
  if Result then
    WriteLegacyUIState('finalizing', '95', 'Underground 2 MOVIES package installed.')
  else
  begin
...
begin
  ...
  begin
    ...
    WriteLegacyUIState('removing', '25', 'Removing installed Underground 2 Legacy Modpack files...');
    DeleteFilesFromManifest(GameDir, NewFilesManifestPath);

    if DirExists(BackupDir) then
    begin
      WriteLegacyUIState('restoring', '60', 'Restoring original Underground 2 game files...');
      ...
      WriteLegacyUIState('complete', '100', 'Rollback complete. Original Underground 2 files restored.');
    end
    else
    begin
