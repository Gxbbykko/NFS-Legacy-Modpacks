#define MyAppName "Undercover Legacy Modpack"
#define MyAppVersion "1.0.0.1"
#define MyAppPublisher "Gxbbykko"
#define MyOutputName "UndercoverMP"

#define GameId "nfsuc"
#define GameExe "nfs.exe"
#define ArchiveName "NFSUC.arc"
#define TempExtractFolder "UndercoverLegacy_Extract"

#define MoviesUrl "https://drive.usercontent.google.com/download?id=1E--_v6Q13_y7Tzo5vqkO69CqR6MA-eMv&export=download&confirm=t&uuid=08da197d-185f-41af-ba7d-41ce3d89d830"
#define MoviesArchiveName "NFSUC_MOVIES.7z"
#define MoviesExtractFolder "NFSUCMoviesExtract"
#define MoviesSourceSubDir "NFSUC\MOVIES"
#define MoviesExpectedMinSize 950000000

#define ProjectRoot "C:\Users\Gabriel\Desktop\NFSUC_Modpack"
...
[Setup]
AppId={{5F1C8E3D-9A24-44C1-BB0A-51C8D6B74E92}
AppName={#MyAppName}
...
SetupIconFile={#ImagesDir}\nfs_icon.ico

Uninstallable=yes
CreateUninstallRegKey=yes
UninstallFilesDir={app}\_LegacyInstaller
UninstallDisplayName=Undercover Legacy Modpack Restore Tool
...
[Icons]
Name: "{app}\_LegacyInstaller\Restore Undercover Legacy Modpack"; Filename: "{app}\_LegacyInstaller\LegacyUI\LegacyUI.exe"; Parameters: "--target ""{app}"" --mode uninstall --game {#GameId}"; WorkingDir: "{app}\_LegacyInstaller\LegacyUI"; IconFilename: "{app}\_LegacyInstaller\LegacyUI\LegacyUI.exe"
...
var
  ...
  InstallFilterOffMovies: Boolean;

function GetDefaultDir(Param: String): String;
begin
  if FileExists('C:\Games\Need for Speed - Undercover\{#GameExe}') then
    Result := 'C:\Games\Need for Speed - Undercover'
  else if FileExists(ExpandConstant('{pf}\Electronic Arts\Need for Speed Undercover\{#GameExe}')) then
    Result := ExpandConstant('{pf}\Electronic Arts\Need for Speed Undercover')
  else if FileExists(ExpandConstant('{pf32}\Electronic Arts\Need for Speed Undercover\{#GameExe}')) then
    Result := ExpandConstant('{pf32}\Electronic Arts\Need for Speed Undercover')
  else if FileExists(ExpandConstant('{pf}\EA GAMES\Need for Speed Undercover\{#GameExe}')) then
    Result := ExpandConstant('{pf}\EA GAMES\Need for Speed Undercover')
  else if FileExists(ExpandConstant('{pf32}\EA GAMES\Need for Speed Undercover\{#GameExe}')) then
    Result := ExpandConstant('{pf32}\EA GAMES\Need for Speed Undercover')
  else
    Result := 'C:\Games\Need for Speed - Undercover';
end;
...
begin
  Result := AddBackslash(ExpandConstant('{src}')) + '_UndercoverLegacy_Error_Backup';
end;
...
begin
  ...
  SaveStringToFile(
    LogFile,
    'Undercover Legacy Modpack installation error'#13#10 +
...
begin
  ...
  begin
    ...
    begin
      ...
      MoviesValue := ReadCommandValue(CommandText, 'movies');

      InstallFilterOffMovies :=
        CompareText(MoviesValue, 'filteroff') = 0;
      ...
      if InstallFilterOffMovies then
        WriteLegacyUIState('preparing', '8', 'Install command received. Filter Off MOVIES package selected.')
      else
        WriteLegacyUIState('preparing', '8', 'Install command received. Vanilla MOVIES will be kept.');
      Result := True;
      Exit;
    end;
  end;
...
begin
  ...
  WriteLegacyUIState('extracting', '20', 'Extracting Undercover archive payload...');
  ...
  repeat
    ...
    ProgressPercent := Integer(ExtractedSize div 15969271);
    ...
    WriteLegacyUIState('extracting', IntToStr(ProgressPercent), 'Extracting Undercover archive payload...');
  ...
  WriteLegacyUIState('copying', '65', 'Installing Undercover modpack files...');
  Result := True;
end;
...
procedure AppendDirectoryToManifest(SourceDir, BaseDir, Prefix, ManifestPath: String);
var
  FindRec: TFindRec;
  SourcePath: String;
  RelPath: String;
begin
  SourceDir := RemoveBackslashUnlessRoot(SourceDir);
  BaseDir := RemoveBackslashUnlessRoot(BaseDir);
    ...
    try
      repeat
        ...
        begin
          ...
          if (FindRec.Attributes and FILE_ATTRIBUTE_DIRECTORY) <> 0 then
          begin
            AppendDirectoryToManifest(SourcePath, BaseDir, Prefix, ManifestPath);
          end
          else
          begin
            RelPath := Copy(SourcePath, Length(BaseDir) + 2, Length(SourcePath));

            if Prefix <> '' then
              RelPath := AddBackslash(Prefix) + RelPath;
...
begin
  ...
  WriteLegacyUIState('copying', '70', 'Copying extracted files into the Undercover folder...');
  ...
  begin
    ...
    if InstallFilterOffMovies then
      WriteLegacyUIState('finalizing', '76', 'Writing rollback manifest and preparing Filter Off MOVIES package...')
    else
      WriteLegacyUIState('finalizing', '96', 'Writing rollback manifest and finalizing Undercover installation...');
  end
  else
...
begin
  ...
  WriteLegacyUIState('downloading', '78', 'Downloading Undercover Filter Off MOVIES package...');
  ...
  WriteLegacyUIState('downloading', '84', 'Filter Off MOVIES package downloaded successfully.');
  Result := True;
end;
...
begin
  ...
  WriteLegacyUIState('extracting', '86', 'Extracting Undercover Filter Off MOVIES package...');
  ...
  WriteLegacyUIState('extracting', '90', 'Filter Off MOVIES package extracted successfully.');
  Result := True;
end;
...
begin
  ...
  WriteLegacyUIState('copying', '92', 'Installing Undercover Filter Off MOVIES package...');
  ...
  MoviesPackageRoot := ExtractFileDir(SourceMoviesDir);
  ...
  if Result then
    WriteLegacyUIState('finalizing', '95', 'Undercover Filter Off MOVIES package installed.')
  else
  begin
    WriteLegacyUIState('error', '100', 'Failed to install Filter Off MOVIES folder into the game directory.');
    CreateErrorReport('Failed to copy Filter Off MOVIES folder to: ' + DestMoviesDir);
  end;
end;
...
begin
  ...
  if not InstallFilterOffMovies then
  begin
    Result := True;
    Exit;
  end;
...
begin
  ...
  begin
    ...
    WriteLegacyUIState('removing', '25', 'Removing installed Undercover Legacy Modpack files...');
    DeleteFilesFromManifest(GameDir, NewFilesManifestPath);

    if DirExists(BackupDir) then
    begin
      WriteLegacyUIState('restoring', '60', 'Restoring original Undercover game files...');
      ...
      WriteLegacyUIState('complete', '100', 'Rollback complete. Original Undercover files restored.');
    end
    else
    begin
...
begin
  ...
  begin
    ...
    begin
      WriteLegacyUIState('error', '100', 'Filter Off MOVIES package installation failed.');
      CreateErrorReport('Filter Off MOVIES package installation failed.');
      KillInstallerProcesses;
      Abort;
    end;

    WriteLegacyUIState('finalizing', '98', 'Finalizing Undercover installation state...');
  end;
